...
def dashboard(request):
    ...
    evs =expensesVsavings(request.user.id)
    context={
        'data':data,
        'evs':evs
    }
...
def expensesVsavings(id):
        ...
        if total_income is None:
            total_income=0
        expenses = Expenses.objects.filter(user_id=id, date__month=MyCurrentMonth(), date__year=MyCurrentYear()).aggregate(
            Sum('rupees'))
